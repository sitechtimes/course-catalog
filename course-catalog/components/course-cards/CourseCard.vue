...
<template>
    <div v-if="course?.catalog == true">
        <NuxtLink :to="`/courses/${course?.id}`">
            <div id="card" class="bg-zinc-50 w-80 h-36 m-4 px-4 py-[0.6rem] rounded-lg shadow-sm cursor-pointer">
                <h2 id="name" class="text-2xl font-semibold h-[4.4rem]">{{course?.name}}</h2>
